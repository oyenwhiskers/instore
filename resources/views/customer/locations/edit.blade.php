...
@section('page_desc', 'Update outlet details, brand assignment, and products.')
@section('page_actions')
    <a href="{{ route('customer.locations.index') }}" class="btn btn-secondary">Back</a>
@endsection

@section('content')
<form method="POST" action="{{ route('customer.locations.update', $location) }}" class="form-section">
    @csrf
    @method('PUT')
        @php
            $mapsKey = config('services.google.maps_key');
            $mapId = 'location-map-edit';
            $searchId = 'location-search-edit';
        @endphp
        <div class="section-block">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="geofence_radius" value="{{ old('geofence_radius', $location->geofence_radius) }}" class="input" min="0" step="1" placeholder="e.g. 50">
                </div>
            </div>
            <div class="map-shell">
                <div
                    ...
                    data-lat="{{ old('geo_lat', $location->geo_lat) }}"
                    data-lng="{{ old('geo_lng', $location->geo_lng) }}"
                ></div>
            </div>
        ...
        <div class="section-block">
            ...
            <div class="form-grid three-col">
                <div class="form-group">
                    <label>Country</label>
                    <input type="text" name="country" value="{{ old('country', $location->country) }}" class="input" required>
                </div>
                <div class="form-group">
                    <label>State</label>
                    <input type="text" name="state" value="{{ old('state', $location->state) }}" class="input" required>
                </div>
                <div class="form-group">
                    <label>District</label>
                    <input type="text" name="district" value="{{ old('district', $location->district) }}" class="input" required>
                </div>
            </div>
            <div class="form-group">
                <label>Address</label>
