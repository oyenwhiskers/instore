...
@section('page_desc', 'Update event details and assignments.')
...
@section('content')
@php
    $selectedPromoters = old('promoter_ids', $event->promoters->pluck('id')->all());
    $selectedProducts = old('product_ids', $event->products->pluck('id')->all());
    $selectedBrandClients = old(
        'brand_client_ids',
        $event->products->pluck('brand_client_id')->filter()->unique()->values()->all()
    );
    $selectedPremiums = old('premium_ids', $event->premiums->pluck('id')->all());
@endphp
<form method="POST" action="{{ route('customer.events.update', $event) }}" class="form-section">
    @csrf
    @method('PUT')
    ...
    <div class="section-block">
        ...
        <div class="form-grid event-details-grid">
            ...
            <div class="form-group">
                ...
                <input type="date" name="start_date" value="{{ old('start_date', $event->start_date ? \Illuminate\Support\Carbon::parse($event->start_date)->toDateString() : null) }}" class="input" required>
            </div>
            <div class="form-group">
                <label>End Date</label>
                <input type="date" name="end_date" value="{{ old('end_date', $event->end_date ? \Illuminate\Support\Carbon::parse($event->end_date)->toDateString() : null) }}" class="input" required>
            </div>
        </div>
        <div class="form-group">
            <label>Notes</label>
            <textarea name="notes" class="textarea" rows="2">{{ old('notes', $event->notes) }}</textarea>
        </div>
    </div>
    ...
    <div class="section-block">
        ...
        <div class="event-promoter-grid">
            @foreach ($promoters as $promoter)
                @php
                    $pivot = $event->promoters->firstWhere('id', $promoter->id)?->pivot;
                @endphp
                <div class="event-promoter-card">
                    ...
                    <div class="event-promoter-fields">
                        <div class="form-group">
                            <label>Start Date</label>
                            <input type="date" name="promoter_schedule[{{ $promoter->id }}][start_date]" value="{{ old('promoter_schedule.' . $promoter->id . '.start_date', $pivot?->start_date ? \Illuminate\Support\Carbon::parse($pivot->start_date)->toDateString() : null) }}" class="input">
                        </div>
                        <div class="form-group">
                            <label>End Date</label>
                            <input type="date" name="promoter_schedule[{{ $promoter->id }}][end_date]" value="{{ old('promoter_schedule.' . $promoter->id . '.end_date', $pivot?->end_date ? \Illuminate\Support\Carbon::parse($pivot->end_date)->toDateString() : null) }}" class="input">
                        </div>
                        <div class="form-group">
                            <label>Start Time</label>
                            <input type="time" name="promoter_schedule[{{ $promoter->id }}][start_time]" value="{{ old('promoter_schedule.' . $promoter->id . '.start_time', $pivot?->start_time) }}" class="input">
                        </div>
                        <div class="form-group">
                            <label>End Time</label>
                            <input type="time" name="promoter_schedule[{{ $promoter->id }}][end_time]" value="{{ old('promoter_schedule.' . $promoter->id . '.end_time', $pivot?->end_time) }}" class="input">
                        </div>
                    </div>
                </div>
            @endforeach
    ...
    <div class="section-block">
        ...
        <div class="form-grid compact">
            ...
            <div class="form-group">
                ...
                <input type="text" id="event-product-search" class="input" placeholder="Search product or SKU">
            </div>
        </div>
        <div class="form-grid">
